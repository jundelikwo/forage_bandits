...
def main(cfg: DictConfig) -> None:  # pragma: no cover
    ...
    hazard, regret, exploring, lifetimes = run_simulation(cfg, "discountedegree" if is_discounted else "egree", False, eta=0)
    print("  ε-Greedy (energy)...")
    hazard_ea, regret_ea, exploring_ea, lifetimes_ea = run_simulation(cfg, "discountedegree" if is_discounted else "egree", True)
    ...
    # UCB
    print("  UCB (no energy)...")
    hazard, regret, exploring, lifetimes = run_simulation(cfg, "discounteducb" if is_discounted else "ucb", False, eta=0)
    print("  UCB (energy)...")
    hazard_ea, regret_ea, exploring_ea, lifetimes_ea = run_simulation(cfg, "discounteducb" if is_discounted else "ucb", True)
    ...
    # TS
    print("  TS (no energy)...")
    hazard, regret, exploring, lifetimes = run_simulation(cfg, "discountedts" if is_discounted else "ts", False, eta=0)
    print("  TS (energy)...")
    hazard_ea, regret_ea, exploring_ea, lifetimes_ea = run_simulation(cfg, "discountedts" if is_discounted else "ts", True)
