...
def main(cfg: DictConfig) -> None:
    ...
    for epsilon in epsilon_range:
        ...
        regret, regret_std, lifetime, lifetime_std = run_simulation(cfg, epsilon, "egree", False, eta=0)
        results["e_greedy_no_energy"]["lifetime"][epsilon] = lifetime
        results["e_greedy_no_energy"]["lifetime_std"][epsilon] = lifetime_std
        results["e_greedy_no_energy"]["regret"][epsilon] = regret
        results["e_greedy_no_energy"]["regret_std"][epsilon] = regret_std

        print("  ε-Greedy (no energy, eta=1)...")
        regret, regret_std, lifetime, lifetime_std = run_simulation(cfg, epsilon, "egree", False, eta=1)
        results["e_greedy_no_energy_eta_1"]["lifetime"][epsilon] = lifetime
        results["e_greedy_no_energy_eta_1"]["lifetime_std"][epsilon] = lifetime_std
        results["e_greedy_no_energy_eta_1"]["regret"][epsilon] = regret
        results["e_greedy_no_energy_eta_1"]["regret_std"][epsilon] = regret_std
        
        print("  ε-Greedy (energy)...")
        regret, regret_std, lifetime, lifetime_std = run_simulation(cfg, epsilon, "egree", True, eta=1)
        results["e_greedy_energy"]["lifetime"][epsilon] = lifetime
        results["e_greedy_energy"]["lifetime_std"][epsilon] = lifetime_std
        results["e_greedy_energy"]["regret"][epsilon] = regret
        results["e_greedy_energy"]["regret_std"][epsilon] = regret_std
    ...
    # Plot no energy version
    no_energy = results["e_greedy_no_energy_eta_1"]
    ax.errorbar(
        epsilon_range,
        [no_energy['lifetime'][n] for n in epsilon_range],
        # yerr=[no_energy['lifetime_std'][n] for n in epsilon_range],
    ...
    ax.errorbar(
        epsilon_range,
        [energy['lifetime'][n] for n in epsilon_range],
    ...
    ax.errorbar(
        epsilon_range,
        [energy['regret'][n] for n in epsilon_range],
    ...
    plt.tight_layout()
    plt.savefig(output_dir / f"epsilon_check_{cfg.env.name}_{cfg.env.n_arms}.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    # Save results
    with open(output_dir / f"epsilon_check_{cfg.env.name}_{cfg.env.n_arms}.json", "w") as f:
        json.dump(results, f, indent=2)
    
    print(f"\nResults saved to experiments/results/epsilon_check_{cfg.env.name}_{cfg.env.n_arms}.json")
    print(f"Plot saved to experiments/results/epsilon_check_{cfg.env.name}_{cfg.env.n_arms}.png")
