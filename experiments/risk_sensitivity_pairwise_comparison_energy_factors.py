...
def main(cfg: DictConfig) -> None:  # pragma: no cover
    ...
    no_energy_label = "Discounted ε-Greedy (no energy)" if is_discounted else "ε-Greedy (no energy)"
    print("  ε-Greedy (no energy)...")
    hazard, regret, exploring, lifetimes, energy = run_simulation(cfg, "discountedegree" if is_discounted else "egree", False, "linear", eta=0)
    ...
    print("  TS (no energy)...")
    hazard, regret, exploring, lifetimes, energy = run_simulation(cfg, "discountedts" if is_discounted else "ts", False, "linear", eta=0)
    ...
    for run in range(2):
        ...
        if plot_energy:
            ...
            ax = plot_lifetime_distribution(lifetimes, ax=axes[lifetime_x, 0], energy_lifetimes=lifetimes_ea, label=no_energy_label, energy_label=energy_label, other_label=other_label, other_lifetimes=lifetimes_other)
            ax.set_xlim(0, 15)
            plot_energy_trajectory(energy, ax=axes[energy_x, 0], label=no_energy_label, energy_label=energy_label, energy_energy=energy_ea, other_label=other_label, other_energy=energy_other)
        ...
        if plot_energy:
            ...
            ax = plot_lifetime_distribution(lifetimes, ax=axes[lifetime_x, 1], energy_lifetimes=lifetimes_ea, label=no_energy_label, energy_label=energy_label, other_label=other_label, other_lifetimes=lifetimes_other)
            ax.set_xlim(0, 15)
            plot_energy_trajectory(energy, ax=axes[energy_x, 1], label=no_energy_label, energy_label=energy_label, energy_energy=energy_ea, other_label=other_label, other_energy=energy_other)
        ...
        if plot_energy:
            ...
            ax = plot_lifetime_distribution(lifetimes, ax=axes[lifetime_x, 2], energy_lifetimes=lifetimes_ea, label=no_energy_label, energy_label=energy_label, other_label=other_label, other_lifetimes=lifetimes_other)
            ax.set_xlim(0, 15)
            plot_energy_trajectory(energy, ax=axes[energy_x, 2], label=no_energy_label, energy_label=energy_label, energy_energy=energy_ea, other_label=other_label, other_energy=energy_other)

        # output_dir = Path("experiments/results")
        # output_dir.mkdir(exist_ok=True)
        # plt.tight_layout()
        # plt.savefig(output_dir / f"risk_sensitivity_pairwise_comparison_energy_factors_{cfg.env.name}_{cfg.env.n_arms}_run{run}{"_discounted" if is_discounted else ""}.png")
