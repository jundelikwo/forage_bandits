...
class NeuralHebbianFactorFunction:
...
def get_neural_hebbian_factor(alpha: float, beta: float, energy_factor_alg: str) -> Callable[[float, bool], float]:
    return NeuralHebbianFactorFunction(alpha, beta, energy_factor_alg)
...
def main(cfg: DictConfig) -> None:
    ...
    # Initialize results dictionaries
    results = {
        "neural_hebbian_no_energy": {"regret": {}, "regret_std": {}, "lifetime": {}, "lifetime_std": {}},
        "neural_hebbian_energy": {"regret": {}, "regret_std": {}, "lifetime": {}, "lifetime_std": {}},
        "neural_hebbian_energy_flip_exp": {"regret": {}, "regret_std": {}, "lifetime": {}, "lifetime_std": {}},
    }
    
    # Run simulations for each number of arms
    for alpha, beta in zipped_range:
        print(f"\nRunning simulations for alpha = {alpha}, beta = {beta}")
        custom_exploration_factor = get_neural_hebbian_factor(alpha, beta, "linear")
        
        # Neural Hebbian
        print("  Neural Hebbian (no energy)...")
        regret, regret_std, lifetime, lifetime_std = run_simulation(cfg, "neural_hebbian", False, eta=0, custom_exploration_function=custom_exploration_factor)
        results["neural_hebbian_no_energy"]["lifetime"][alpha, beta] = lifetime
        results["neural_hebbian_no_energy"]["lifetime_std"][alpha, beta] = lifetime_std
        results["neural_hebbian_no_energy"]["regret"][alpha, beta] = regret
        results["neural_hebbian_no_energy"]["regret_std"][alpha, beta] = regret_std
        
        print("  Neural Hebbian (energy)...")
        regret, regret_std, lifetime, lifetime_std = run_simulation(cfg, "neural_hebbian", True, eta=1, custom_exploration_function=custom_exploration_factor)
        results["neural_hebbian_energy"]["lifetime"][alpha, beta] = lifetime
        results["neural_hebbian_energy"]["lifetime_std"][alpha, beta] = lifetime_std
        results["neural_hebbian_energy"]["regret"][alpha, beta] = regret
        results["neural_hebbian_energy"]["regret_std"][alpha, beta] = regret_std

        print("  Neural Hebbian (energy, flip exp)...")
        regret, regret_std, lifetime, lifetime_std = run_simulation(cfg, "neural_hebbian", True, eta=1, custom_exploration_function=get_neural_hebbian_factor(alpha, beta, "flip_exp"))
        results["neural_hebbian_energy_flip_exp"]["lifetime"][alpha, beta] = lifetime
        results["neural_hebbian_energy_flip_exp"]["lifetime_std"][alpha, beta] = lifetime_std
        results["neural_hebbian_energy_flip_exp"]["regret"][alpha, beta] = regret
        results["neural_hebbian_energy_flip_exp"]["regret_std"][alpha, beta] = regret_std
    
    # Plot results
    configs = ["neural_hebbian_no_energy", "neural_hebbian_energy", "neural_hebbian_energy_flip_exp"]
    titles = ["Non-Energy", "Energy", "Energy, flip_exp"]

    # Create heatmaps for each configuration
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    fig.suptitle(f'Lifetime Heatmaps for Different Neural Hebbian Configurations: {cfg.env.name} environment, n_arms={cfg.env.n_arms}', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / f"neural_hebbian_factor_heatmaps_{cfg.env.name}_{cfg.env.n_arms}.png", dpi=300, bbox_inches='tight')
    plt.close()


    # Create heatmaps for each configuration
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    fig.suptitle(f'Regret Heatmaps for Different Neural Hebbian Configurations: {cfg.env.name} environment, n_arms={cfg.env.n_arms}', fontsize=16)
    ...
    # Adjust layout and save
    plt.tight_layout()
    plt.savefig(output_dir / f"neural_hebbian_factor_heatmaps_regret_{cfg.env.name}_{cfg.env.n_arms}.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    # Save results
    # with open(output_dir / f"neural_hebbian_factor_{cfg.env.name}_{cfg.env.n_arms}.json", "w") as f:
    #     json.dump(results, f, indent=2)
    
    # print(f"\nResults saved to experiments/results/neural_hebbian_factor_{cfg.env.name}_{cfg.env.n_arms}.json")
    print(f"Plot saved to experiments/results/neural_hebbian_factor_heatmaps_{cfg.env.name}_{cfg.env.n_arms}.png")
