...
def get_epsilon_factor(alpha: float, beta: float, energy_factor_alg: str) -> Callable[[float, bool], float]:
    return EpsilonFactorFunction(alpha, beta, energy_factor_alg)
...
def main(cfg: DictConfig) -> None:
    ...
    for alpha, beta in zipped_range:
        ...
        custom_exploration_factor = get_epsilon_factor(alpha, beta, "linear")
        
        # ε-Greedy
        print("  ε-Greedy (no energy)...")
        regret, regret_std, lifetime, lifetime_std = run_simulation(cfg, "egree", False, eta=0, custom_exploration_function=custom_exploration_factor)
        results["e_greedy_no_energy"]["lifetime"][alpha, beta] = lifetime
        results["e_greedy_no_energy"]["lifetime_std"][alpha, beta] = lifetime_std
        results["e_greedy_no_energy"]["regret"][alpha, beta] = regret
        results["e_greedy_no_energy"]["regret_std"][alpha, beta] = regret_std
        
        print("  ε-Greedy (energy)...")
        regret, regret_std, lifetime, lifetime_std = run_simulation(cfg, "egree", True, eta=1, custom_exploration_function=custom_exploration_factor)
        results["e_greedy_energy"]["lifetime"][alpha, beta] = lifetime
        results["e_greedy_energy"]["lifetime_std"][alpha, beta] = lifetime_std
        results["e_greedy_energy"]["regret"][alpha, beta] = regret
        results["e_greedy_energy"]["regret_std"][alpha, beta] = regret_std

        print("  ε-Greedy (energy, flip exp)...")
        regret, regret_std, lifetime, lifetime_std = run_simulation(cfg, "egree", True, eta=1, custom_exploration_function=get_epsilon_factor(alpha, beta, "flip_exp"))
        results["e_greedy_energy_flip_exp"]["lifetime"][alpha, beta] = lifetime
        results["e_greedy_energy_flip_exp"]["lifetime_std"][alpha, beta] = lifetime_std
        results["e_greedy_energy_flip_exp"]["regret"][alpha, beta] = regret
        results["e_greedy_energy_flip_exp"]["regret_std"][alpha, beta] = regret_std
    
    # Plot results
    configs = ["e_greedy_no_energy", "e_greedy_energy", "e_greedy_energy_flip_exp"]
    titles = ["No Energy, eta=0", "Energy", "Energy, flip_exp"]

    # Create heatmaps for each configuration
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    fig.suptitle(f'Lifetime Heatmaps for Different ε-Greedy Configurations: {cfg.env.name} environment, n_arms={cfg.env.n_arms}', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / f"epsilon_factor_heatmaps_{cfg.env.name}_{cfg.env.n_arms}.png", dpi=300, bbox_inches='tight')
    plt.close()


    # Create heatmaps for each configuration
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    fig.suptitle(f'Regret Heatmaps for Different ε-Greedy Configurations: {cfg.env.name} environment, n_arms={cfg.env.n_arms}', fontsize=16)
    ...
    # Adjust layout and save
    plt.tight_layout()
    plt.savefig(output_dir / f"epsilon_factor_heatmaps_regret_{cfg.env.name}_{cfg.env.n_arms}.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    # Save results
    # with open(output_dir / f"epsilon_factor_{cfg.env.name}_{cfg.env.n_arms}.json", "w") as f:
    #     json.dump(results, f, indent=2)
    
    # print(f"\nResults saved to experiments/results/epsilon_factor_{cfg.env.name}_{cfg.env.n_arms}.json")
    print(f"Plot saved to experiments/results/epsilon_factor_heatmaps_{cfg.env.name}_{cfg.env.n_arms}.png")
