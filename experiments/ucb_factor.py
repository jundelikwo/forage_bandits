...
def main(cfg: DictConfig) -> None:
    ...
    plt.tight_layout()
    plt.savefig(output_dir / f"ucb_factor_heatmaps_{cfg.env.name}_{cfg.env.n_arms}.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    # Adjust layout and save
    plt.tight_layout()
    plt.savefig(output_dir / f"ucb_factor_heatmaps_regret_{cfg.env.name}_{cfg.env.n_arms}.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    # Save results
    # with open(output_dir / f"ucb_factor_{cfg.env.name}_{cfg.env.n_arms}.json", "w") as f:
    #     json.dump(results, f, indent=2)
    
    # print(f"\nResults saved to experiments/results/ucb_factor_{cfg.env.name}_{cfg.env.n_arms}.json")
    print(f"Plot saved to experiments/results/ucb_factor_heatmaps_{cfg.env.name}_{cfg.env.n_arms}.png")
