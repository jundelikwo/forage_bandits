...
def main(cfg: DictConfig) -> None:
    ...
    for col, alg in enumerate(['ucb', 'ts']):
        ...
        y_ticks = [30, 35, 40, 45, 50] if cfg.env.name == "sigmoid" else [0, 10, 20, 30, 40, 50]
        ax.set_ylim(np.min(y_ticks), np.max(y_ticks))
        if col == 0:
            ax.set_yticks(y_ticks, labels=y_ticks, fontsize=22)
    ...
    for col, alg in enumerate(['ucb', 'ts']):
        ...
        # Customize subplot
        ax.set_xticks([5, 10, 15], labels=[5, 10, 15], fontsize=22)
        y_ticks = [0, 5, 10, 15, 20, 25] if cfg.env.name == "sigmoid" else [0, 10, 20, 30, 40, 50]
        ax.set_ylim(np.min(y_ticks), np.max(y_ticks))
        if col == 0:
            ax.set_yticks(y_ticks, labels=y_ticks, fontsize=22)
